...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        "✅ Filtered records (servants only):",
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `📝 All servants marked present for ${date} by ${recordedById}`
      );
...
router.delete("/attendance", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    console.log(
      `🗑️ Deleting attendance record for servant ${servantId} on ${date}`
    );
    ...
    console.log(
      `✅ Attendance record deleted successfully for servant ${servantId} on ${date}`
    );
...
router.get(
  ...
  async (req, res) => {
    try {
      console.log(
        "🔍 General servants statistics API called by:",
        req.user.username,
        req.user.role
      );
      ...
      const averageAttendance = Math.round(
        totalServants * (attendanceRate / 100)
      );
      ...
      console.log(
        `📊 General statistics: ${totalServants} total, ${todayPresent} present today, ${servantsNeedingFollowUp} need follow-up`
      );
        ...
        .map((date) => ({
          ...
          attendanceRate:
            dailyStats[date].total > 0
              ? (
                  (dailyStats[date].present / dailyStats[date].total) *
                  100
                ).toFixed(1)
              : 0,
        }))
...
router.get(
  ...
  async (req, res) => {
    try {
      console.log(
        "🔍 Individual servants statistics API called by:",
        req.user.username,
        req.user.role
      );
      ...
      const servantsNeedingFollowUp = servantsWithStats.filter(
        (s) => s.needsFollowUp
      ).length;

      console.log(
        `📊 Found ${servantsNeedingFollowUp} servants needing follow-up out of ${servants.length} total servants`
      );
...
// @desc    Get servants requiring follow-up (absent in last Friday only)
// @access  Protected (Admin or Service Leader)
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Get the most recent Friday only
      const mostRecentFriday = getMostRecentFriday();
      console.log("📅 Most recent Friday:", mostRecentFriday);
      ...
      for (const servant of servants) {
        ...
        // Get attendance record for the most recent Friday only
        ...
        console.log(
          `📊 Friday ${mostRecentFriday} attendance:`,
          attendanceRecord ? attendanceRecord.status : "no record"
        );

        // If no record or status is absent, add to follow-up list
        if (!attendanceRecord || attendanceRecord.status === "absent") {
          // Get last present date for display
          const lastPresentRecord = await Attendance.findOne({
            person: servant._id,
            personModel: "User",
            status: "present",
          }).sort({ date: -1 });
          ...
          servantsNeedingFollowUp.push({
            ...
            name: servant.name,
            phone: servant.phone,
            lastAbsenceDate: mostRecentFriday,
            lastPresentDate: lastPresentRecord ? lastPresentRecord.date : null,
            status: attendanceRecord ? attendanceRecord.status : "no_record",
          });
        }
      }

      console.log(
        `📊 Found ${servantsNeedingFollowUp.length} servants needing follow-up from last Friday`
      );

      res.json({
        success: true,
        data: servantsNeedingFollowUp,
        criteria: "Absent on last Friday",
...
router.get(
  ...
  async (req, res) => {
    try {
      console.log(
        "🔍 Servants by class statistics API called by:",
        req.user.username,
        req.user.role
      );
      ...
      for (const classItem of classes) {
        ...
        classStats.push({
          ...
          servantsNeedingFollowUp: servantsWithStats.filter(
            (s) => s.needsFollowUp
          ).length,
        });
      }
      ...
      res.json({
        ...
        totalServantsChecked: classStats.reduce(
          (sum, cls) => sum + cls.totalServants,
          0
        ),
      });
    } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        "🔍 Individual servant statistics API called for:",
        servantId
      );
      ...
      const recentActivity = attendanceRecords.slice(0, 10).map((record) => ({
        date: record.date,
        status: record.status,
        dayName: new Date(record.date + "T00:00:00").toLocaleDateString(
          "ar-EG",
          { weekday: "long" }
        ),
        notes: record.notes || "",
      ...
      for (let month = 1; month <= 12; month++) {
        const monthStart = `${currentYear}-${month
          .toString()
          .padStart(2, "0")}-01`;
        const monthEnd = `${currentYear}-${month
          .toString()
          .padStart(2, "0")}-31`;
      ...
      console.log(
        `📊 Individual statistics compiled for servant: ${servant.name}`
      );
...
// @route   POST /api/servants/follow-up/complete
// @desc    Mark servant follow-up as completed (remove from follow-up list)
// @access  Protected (Admin or Service Leader)
router.post(
  "/follow-up/complete",
  authMiddleware,
  ...
  async (req, res) => {
    try {
      const { servantId, notes } = req.body;

      if (!servantId) {
        return res.status(400).json({
          success: false,
          error: "Servant ID is required",
        });
      }
      ...
      // Mark attendance as "excused" for the most recent Friday to remove from follow-up list
      // This means the servant was contacted and follow-up is complete
      let attendance = await Attendance.findOne({
        person: servantId,
        personModel: "User",
        date: mostRecentFriday,
      ...
      if (attendance) {
        // Update existing attendance to "excused" (follow-up completed)
        attendance.status = "excused";
        attendance.notes = notes || "تم الافتقاد";
        attendance.recordedBy = req.user._id;
        attendance.updatedAt = new Date();
        await attendance.save();
      } else {
      ...
      console.log(
        `✅ Follow-up completed for servant: ${servant.name} on ${mostRecentFriday}`
      );

      res.json({
        success: true,
        message: "تم تسجيل الافتقاد بنجاح",
        data: {
      ...
      console.error("❌ Error in follow-up complete:", error);
      res.status(500).json({
        success: false,
        error: "Server error",
      });
